...
**CJ_AI_개발방법론**은 AI(Claude Code) 시대에 맞춘 **사고방식 + 실행방법**을 통합한 소프트웨어 개발 프레임워크입니다.
...
🎯 Layer 0: Product (제품)
   └─ 3 Blocks
       ...
       │       └─ ⚙️ Feature 3 (5 Tasks)
       │
       ├─ 📦 Block 2 (3 Features, 15 Tasks)
       └─ 📦 Block 3 (3 Features, 15 Tasks)

총 구성: 1 제품 = 3 블럭 = 9 중단위 = 45 작은단위
```

**권장 구성**:
- 1개 제품 = **3개 블럭** (Miller's Law: 7±2)
- 1개 블럭 = **3개 중단위(Feature)** (Miller's Law: 7±2)
- 1개 중단위 = **5개 작은단위(Task)** (Miller's Law: 7±2)
- 1개 작은단위 = **1-2시간 작업** (인지 부하 관리)
...
> **실제 개발 방향**: Task 개발(⬇️) → Feature TDD(⬆️) → Block TDD(⬆️) → Product E2E TDD(⬆️)

```
Product E2E Test ⬆️ (Block 3개 완료 후 작성)
  ↑
Block Module Test ⬆️ (Feature 3개 완료 후 작성)
  ↑
Feature Integration Test ⬆️ (Task 5개 완료 후 작성)
  ↑
...
- **Feature Integration TDD**: Task 5개 완료 후 작성 (⬆️ 상향) - Task 개발 중 작성 ❌
- **Block Module TDD**: Feature 3개 완료 후 작성 (⬆️ 상향) - Feature 개발 중 작성 ❌
- **Product E2E TDD**: Block 3개 완료 후 작성 (⬆️ 상향) - Block 개발 중 작성 ❌

```mermaid
graph TD
    subgraph "Layer 0: Product"
        P[Product PRD] --> PE[E2E Test ⬆️<br/>Block 3개 완료 후 작성]
    end

    subgraph "Layer 1: Block"
        B[Block 설계] --> BM[Module Test ⬆️<br/>Feature 3개 완료 후 작성]
    end

    subgraph "Layer 2: Feature"
        F[Feature 설계] --> FI[Integration Test ⬆️<br/>Task 5개 완료 후 작성]
    end
...
**핵심 내용:**
- 📋 계층 구조 (3 Blocks → 9 Features → 45 Tasks)
...
3 Blocks, 9 Features, 45 Tasks로 분해하고 E2E Test 계획도 포함해줘."
...
**A**: 아니요. 작업 규모에 따라 선택적 적용하세요.

- **단순 버그** (5분 이내): TDD만 (Red-Green-Refactor)
- **중간 복잡도** (30분~2시간): 5단계 간단히 + TDD
- **복잡한 기능** (1일 이상): 5단계 전체 + TDD
...
**마지막 업데이트:** 2025-11-07 (계층적 구조 4-Layer 섹션 추가)
**다음 리뷰:** 2025-12-07 (1개월 후)
